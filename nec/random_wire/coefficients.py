"""Collapse the per-group fits into the table the page will carry.
...
#: installation reaches: 2 m on 160 m at the bottom, 25 m on 10 m at top.
NODES = np.array([0.012, 0.03, 0.06, 0.12, 0.25, 0.5, 1.0, 2.5])
...
if __name__ == "__main__":
    ...
    table = build_table(groups, len(soils))
    ...
    print("\n// Fitted coefficients, from nec/random_wire/coefficients.py.")
    print(
        f"const MODEL_H_NODES = Object.freeze([{', '.join(f'{n:g}' for n in NODES)}]);"
    )
    print("const MODEL_COEFFS = Object.freeze({")
    for si, soil in enumerate(soils):
        print(f"  {soil}: Object.freeze({{")
